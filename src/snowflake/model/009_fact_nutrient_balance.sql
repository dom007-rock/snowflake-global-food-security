...
CREATE TABLE IF NOT EXISTS FACT_NUTRIENT_BALANCE (
...
MERGE INTO FACT_NUTRIENT_BALANCE AS target
...
USING (
    ...
    SELECT
        ...
        esb.flag_code,
        esb.flag_description,
        esb.note,

        esb.source_row_hash,
        esb.ingestion_run_id,
        esb.ingestion_batch_id,
        esb.extracted_at_utc

    FROM GFS_DEV.CLEAN.FAOSTAT_ESB esb

    JOIN DIM_GEOGRAPHY g
        ON esb.area_code_fao = g.area_code_fao

    JOIN DIM_ITEM i
        ON esb.source_domain = i.source_domain
       AND esb.item_code = i.item_code

    JOIN DIM_ELEMENT e
        ON esb.source_domain = e.source_domain
       AND esb.element_code = e.element_code
