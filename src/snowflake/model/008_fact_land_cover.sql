...
CREATE TABLE IF NOT EXISTS FACT_LAND_COVER (
...
MERGE INTO FACT_LAND_COVER AS target
...
USING (
    ...
    SELECT
        ...
        lc.source_row_hash,
        lc.ingestion_run_id,
        lc.ingestion_batch_id,
        lc.extracted_at_utc

    FROM GFS_DEV.CLEAN.FAOSTAT_LC lc

    JOIN DIM_GEOGRAPHY g
        ON lc.area_code_fao = g.area_code_fao

    JOIN DIM_ITEM i
        ON lc.source_domain = i.source_domain
       AND lc.item_code = i.item_code

    JOIN DIM_ELEMENT e
        ON lc.source_domain = e.source_domain
       AND lc.element_code = e.element_code
